...
def _empty_queue(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(serve, "_state", {"running": None, "waiting": [], "last": None})
    monkeypatch.setattr(serve.cases, "every", lambda: ["system4", "hanget-soi", "song-s3"])


def test_a_tier_that_is_not_a_tier_is_refused() -> None:
...
def test_a_case_this_host_does_not_have_is_refused() -> None:
    assert "error" in serve.enqueue("one", ["../../etc/passwd"])
    assert "error" in serve.enqueue("one", ["not-a-case"])
    assert serve.snapshot()["waiting"] == []


def test_a_single_case_run_needs_a_case() -> None:
    assert "error" in serve.enqueue("one", [])


def test_a_bulk_tier_ignores_any_names_sent_with_it() -> None:
...
def test_presses_queue_rather_than_collide() -> None:
...
def test_what_is_running_and_what_finished_are_both_said() -> None:
...
def test_the_page_carries_the_controls_and_hides_them_without_a_runner() -> None:
