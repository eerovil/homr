# ruff: noqa: S607 -- fixed git commands in disposable test repositories
...
from fixturecheck import cases, quality, references, series
from fixturecheck.compare import Result, Row


def case(name: str, **counts: int) -> series.CaseRecord:
    full = dict.fromkeys(series.COUNTS, 0)
    full.update(counts)
    return series.CaseRecord(name, counts=full)


def _stands(name: str, ok: bool) -> series.CaseRecord:
...
def test_a_small_run_does_not_destroy_a_large_one(tmp_path: Path) -> None:
    ...
    series.record_run(
        "fixturecheck", "one", [case("song-s0", agree=9, pitch=1)], references="ref1", path=big
    )
...
def test_a_run_names_both_the_homr_and_the_references(tmp_path: Path) -> None:
    """A score that improved because a reference was fixed is not homr improving."""
    path = tmp_path / "series.jsonl"
    run = series.record_run(
        "fixturecheck", "ten", [case("a", agree=1)], references="deadbeef", path=path
    )
    assert run["references"] == "deadbeef"
    assert run["homr"]  # whatever this checkout is, it is recorded


def test_losing_notes_and_shifting_beats_count_against_the_score() -> None:
...
def test_nothing_computes_perfection_any_more() -> None:
...
def test_a_case_that_could_not_be_read_is_recorded_not_skipped(tmp_path: Path) -> None:
    ...
    assert run["outcomes"] == {series.READ: 1, series.UNREADABLE: 1, series.UNBUILDABLE: 1}
...
def test_only_the_disagreements_are_kept(tmp_path: Path) -> None:
...
def test_an_unchanged_fixture_table_is_recorded_by_reference(tmp_path: Path) -> None:
    ...
    first = series.record_run(
        "fixturecheck",
        "one",
        [series.CaseRecord("system4", counts=dict.fromkeys(series.COUNTS, 0), rows=table)],
        references="r",
        path=path,
    )
    ...
    series.record_run(
        ...
        [
            series.CaseRecord(
                "system4", counts=dict.fromkeys(series.COUNTS, 0), rows_same_as=from_run
            )
        ],
        references="r",
        path=path,
...
def test_a_damaged_line_does_not_take_the_series_with_it(tmp_path: Path) -> None:
    path = tmp_path / "series.jsonl"
    series.record_run("fixturecheck", "one", [case("a", agree=1)], references="r", path=path)
    with path.open("a") as writing:
        writing.write("{not json\n")
    series.record_run("fixturecheck", "one", [case("b", agree=1)], references="r", path=path)
...
def test_a_reference_that_moves_is_visible_in_the_run_key(tmp_path: Path) -> None:
...
def test_a_reference_nobody_froze_is_not_silently_the_manifest(tmp_path: Path) -> None:
    manifest = tmp_path / "references.json"
...
def test_freezing_a_few_does_not_drop_the_rest(tmp_path: Path) -> None:
...
def test_the_summary_names_which_homr_and_says_what_it_cannot_measure(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
    path = tmp_path / "series.jsonl"
...
def test_the_summary_names_the_cases_that_fell(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
    path = tmp_path / "series.jsonl"
...
def test_the_two_harnesses_are_reported_apart(tmp_path: Path) -> None:
    """They score different things; one figure over both would mean nothing."""
    path = tmp_path / "series.jsonl"
    series.record_run(
        "fixturecheck", "all", [case("a", agree=8, pitch=2)], references="r", path=path
    )
    series.record_run(
        "choir-bench", "benchmark", [case("b", agree=5, pitch=5)], references="r", path=path
    )

    fixture_run = series.latest("fixturecheck", path)
...
def test_a_run_with_nothing_remembered_in_it_records_no_gate(
    ...
    monkeypatch.setattr(references, "accepted", lambda *a, **k: _accepted("song-s1", score=90.0))
...
def test_nothing_recorded_is_not_a_passing_gate(tmp_path: Path) -> None:
    path = tmp_path / "series.jsonl"
    series.record_run(
        "fixturecheck", "one", [case("song-s1", agree=5)], references="r", gate=None, path=path
    )
    written = quality.render(path)
...
def _fixture_run(
    path: Path,
    cases_: list[series.CaseRecord],
    at_committed: tuple[str, ...] = ("a", "b", "c", "d", "e"),
    **kw: Any,
) -> dict:
    return series.record_run(
        "fixturecheck",
        kw.pop("tier", "one"),
        cases_,
        references="r",
...
def test_the_published_gate_is_each_cases_own_latest_result(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
...
def test_a_one_case_run_still_reports_every_case(
...
def _under(
    ...
    return series.record_run(
        ...
        homr=homr,
        extra={"committed": sorted(roster)},
        path=path,
    )


def test_a_pass_under_one_homr_is_not_a_pass_under_the_next(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
...
def test_a_pass_does_not_survive_the_references_moving(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
    """The same hole, through the other half of the identity."""
    path = tmp_path / "series.jsonl"
    remembered(_accepted(*"abcde"))
    _under(path, [_stands(n, True) for n in "abcde"], homr="A", references="refA", tier="all")
    gate = series.published_gate(series.runs(path))
    assert gate is not None
    assert gate["passed"]
...
def test_standing_results_are_kept_apart_by_what_measured_them(tmp_path: Path) -> None:
    path = tmp_path / "series.jsonl"
    _under(path, [_stands(n, True) for n in "abcde"], homr="A", tier="all")
    _under(path, [_stands("a", True)], homr="B")
...
def test_the_report_does_not_count_rows_from_another_homr(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_a_fixture_nobody_has_judged_holds_the_gate_open(
    tmp_path: Path, remembered: Callable[[dict], None]
) -> None:
...
def test_the_benchmark_records_the_engine_it_measured_not_this_checkout(tmp_path: Path) -> None:
    ...
    head = sp.run(
        ["git", "log", "-1", "--format=%h"], cwd=tree, capture_output=True, text=True, check=True
    ).stdout.strip()
...
def test_the_ordinary_run_finds_the_install_without_being_told(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    (venv / "bin" / "homr").write_text("#!/bin/sh\n")
    (site / "direct_url.json").write_text(
        json.dumps({"vcs_info": {"commit_id": "ec4155991a5d7c17b8049dec2a690c86cba7db96"}})
    )
...
def test_no_install_anywhere_is_unknown_rather_than_a_guess(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("HOMR_BIN", raising=False)
...
def test_a_worktree_is_keyed_by_its_homr_code_not_by_its_head(tmp_path: Path) -> None:
    ...
    def commit(message: str) -> str:
        sp.run(["git", "add", "-A"], cwd=tree, check=True)
        sp.run(  # noqa: S603 -- message is a fixed test string, not shell input
...
def test_an_explicit_revision_beats_the_checkout(tmp_path: Path) -> None:
    path = tmp_path / "series.jsonl"
    run = series.record_run(
        "choir-bench",
        "benchmark",
...
def test_an_unread_case_carries_no_counts(outcome: str) -> None:
...
def test_the_page_says_which_series_it_was_rendered_from(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_two_checkouts_that_share_a_name_are_not_one_history(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_a_branch_switch_under_one_path_is_seen(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_a_history_rewritten_after_the_last_render_is_seen(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_a_run_changed_in_the_middle_of_the_shown_history_is_seen(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_an_ordinary_append_says_nothing(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_a_damaged_marker_does_not_stop_the_render(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    from fixturecheck import report
