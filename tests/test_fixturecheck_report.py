...
def _entry(name: str, **over) -> dict:
    entry = {"name": name, "page": "", "score": 100.0, "agree": 10,
...
def _index(tmp_path, monkeypatch, entries):
...
def test_committed_fixtures_get_their_own_table(tmp_path, monkeypatch):
    page = _index(tmp_path, monkeypatch,
                  [_entry("hanget-soi", agree=7, pitch=3),
                   _entry("some-song-s1", agree=5, voice=2)])
...
def test_no_fixture_section_when_none_measured(tmp_path, monkeypatch):
