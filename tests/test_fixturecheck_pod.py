...
def fresh_pod(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        pod, "reachable", lambda: pytest.fail("the test did not say whether the cluster answers")
    )


@pytest.fixture
def case(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> cases.Case:
    ...
    return cases.Case(
        name="little", image=image, reference=tmp_path / "little.ref.musicxml", origin="song"
    )


def reader(tmp_path: Path, script: str) -> str:
...
def test_off_asks_the_cluster_nothing(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CHOIR_K8S", "off")
    monkeypatch.setattr(
        subprocess,
        "run",
        lambda *a, **k: pytest.fail("CHOIR_K8S=off ran a subprocess on the way to saying no"),
    )
    assert pod.shim() is None
    assert pod.tag() == ""


def test_an_unanswered_cluster_falls_back_out_loud(
...
def test_require_refuses_to_read_here(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_a_pod_parse_is_cached_under_its_own_name(
...
def test_losing_the_pod_midrun_reads_the_case_here(
    tmp_path: Path,
    case: cases.Case,
...
def test_a_page_homr_cannot_read_is_still_none(
    tmp_path: Path, case: cases.Case, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_losing_the_pod_under_require_stops_the_run(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CHOIR_K8S", "require")
    with pytest.raises(SystemExit):
        pod.lose()
