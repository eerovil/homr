...
def test_each_span_between_signatures_gets_its_own_numerator() -> None:
    ...
    assert [int(n * d) for n, d in zip(found, (4, 4, 2), strict=True)] == [3, 5, 5]


def test_one_median_for_the_whole_voice_could_not_do_that() -> None:
...
def test_an_unchanging_meter_is_unaffected() -> None:
...
def test_a_span_with_no_complete_bar_falls_back() -> None:
...
def test_a_voice_with_no_signature_at_all_asks_for_nothing() -> None:
    assert find_nominator_per_time_signature(bar(4) + bar(4), Fraction(1)) == []


def test_the_state_hands_them_out_in_order_then_falls_back() -> None:
