...
def test_confidence_reports_selected_token_alternatives_and_margin() -> None:
    report = confidence_for_logits(np.array([[0.0, 2.0, 1.0]]), {0: "32nd", 1: "16th", 2: "eighth"})
    ...
    assert (
        report["margin"]
        == report["alternatives"][0]["probability"] - report["alternatives"][1]["probability"]
    )
