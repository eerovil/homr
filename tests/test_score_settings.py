...
def test_minimum_duration_rejects_32nds_but_keeps_dotted_16ths_and_tuplets() -> None:
...
def test_settings_can_forbid_tuplets_and_grace_notes() -> None:
    forbidden = RhythmSettings.from_json(
...
def test_stem_voice_hints_are_on_unless_a_score_turns_them_off() -> None:
    assert RhythmSettings.from_json({}).stem_voice_hints
    assert RhythmSettings.from_json({"postprocessing": {"stem_voice_hints": True}}).stem_voice_hints
